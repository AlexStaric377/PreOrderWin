...
namespace PreOrderWin
{
    ...
    // Таблица Справочник взаимозаменяемых составляющих в хот доге 
    public class HotDogUnit
    {
        public int Id { get; set; }
        public int Kodtov { get; set; }
        public int KodUnit { get; set; }
        public string NameUnitUk { get; set; }
        public string NameUnitEn { get; set; }
    ...
    public class ApplicationContext : DbContext
    {
        ...
        public DbSet<HotDogUnit> HotDogUnits { get; set; }
        ...
        public   ApplicationContext()
        {
            ...
            while (Open < 8)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    //MessageError NewOrder = new MessageError(TextWindows, 1, 4);
                    //NewOrder.ShowDialog();
                }
                Open++;
            }
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            ...
            if (File.Exists(PuthServer + "PreOrder.conf"))
            {
                ...
                foreach (string x in fileLines)
                {
                    ...
                    {
                        if (x.Trim().StartsWith("DbPuth")) DbPuth = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1));
                        if (x.Trim().StartsWith("PortTerminal"))
                        {
                            MainWindow.PortTerminal = "PortTerminal";
                            if(x.Trim().Length> x.IndexOf("=")+1) MainWindow.USBPortTerminal = Convert.ToByte(x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)));
                        }
                        if (x.Trim().StartsWith("RunTerminal")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.RunTerminal = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim() == "true" ? true : false;
                        if (x.Trim().StartsWith("RunServer")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.RunServer = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim() == "true" ? true : false;
                        if (x.Trim().StartsWith("TcpUrlServer"))if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.TcpUrlServer = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim();
                        if (x.Trim().StartsWith("PortUrlServer")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.PortUrlServer = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim();
                        if (x.Trim().StartsWith("TimeEndSmena")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.TimeEndSmena = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim();
                        if (x.Trim().StartsWith("TimeEndRegOrder")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.TimeEndRegOrder = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim();
                        if (x.Trim().StartsWith("CodTexnologLoyalnost")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.CodTexnologLoyalnost = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim(); // kod = 9010009026414
                        if (x.Trim().StartsWith("MsgLoyalty")) if (x.Trim().Length > x.IndexOf("=") + 1) MainWindow.MsgLoyaltyOnOff = x.Substring(x.IndexOf("=") + 1, x.Length - (x.IndexOf("=") + 1)).Trim() == "true" ? true : false;
            ...
            PreOrderConection.ErorDebag("Установка соединения. "+ DbPuth, 2);
    ...
    public class TovSelected  //: INotifyPropertyChanged
    {


        public TovSelected(int Id, int KodTov, decimal PriceTov, string Name, byte[] ImagePath)
        {
            this.Id = Id;
            this.KodTov = KodTov;
            this.PriceTov = PriceTov;
    ...
    //    public Dictionary<string, Tovar> goods { get; set; }
    ...
    public class Tovardiscountbonus
    {
        public string code { get; set; }
        public decimal qty { get; set; }
        public decimal price { get; set; }
